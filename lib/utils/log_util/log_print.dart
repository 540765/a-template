...
class ConsolePrint extends IHCLogPrint {
  @override
  void logPrint(
    ...
    for (String str in strList) {
      ...
      for (int i = 0; i < str.length; i++) {
        ...
        len += str[i].contains(chineseRegex) ? 3 : 1;

        ///寻找当前字符的下一个字符长度
        int next = (i + 1) < str.length
            ? str[i + 1].contains(chineseRegex)
                ? 3
                : 1
            : 0;

        ///当前字符累计长度 如果达到了需求就清空
        current += str[i].contains(chineseRegex) ? 3 : 1;
        if (current < maxCharLength && (current + next) >= maxCharLength) {
          entry.add(i);
          current = 0;
        }
      }
